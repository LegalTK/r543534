...
namespace detours {
	...
	static bool updatingClientAnimation = false;

	struct AnimationUpdateScope {
		CGlobalVars& vars;
		CBasePlayer& player;
		double curtime;
		float frametime;
		...
		AnimationUpdateScope(CBasePlayer& target, double time, int tick, double ticks)
			: vars(*interfaces::globalVars), player(target), curtime(vars.curtime),
			frametime(vars.frametime), tickcount(vars.tickcount),
			interpolation(vars.interpolation_amount), clientSideAnimation(player.m_bClientSideAnimation()) {
			updatingClientAnimation = true;
			vars.curtime = time;
			vars.frametime = static_cast<float>(vars.interval_per_tick * ticks);
			vars.tickcount = tick;
			vars.interpolation_amount = 0.0f;
			player.m_bClientSideAnimation() = true;
		}

		~AnimationUpdateScope() {
			player.m_bClientSideAnimation() = clientSideAnimation;
			vars.curtime = curtime;
			vars.frametime = frametime;
			vars.tickcount = tickcount;
			vars.interpolation_amount = interpolation;
			updatingClientAnimation = false;
		}
	};

	static bool rebuildAnimationBones(CBasePlayer* player) {
		auto* lua = interfaces::clientLua;
		player->PushEntity();

		for (const char* method : { "InvalidateBoneCache", "SetupBones" }) {
			...
			if (lua->PCall(1, 0, 0) != 0) {
				static_cast<GarrysMod::Lua::ILuaInterface*>(lua)->ErrorNoHalt("%s\n", lua->GetString(-1));
				lua->Pop(2);
				return false;
			}
		}
	...
	bool updateClientSideAnimations(CBasePlayer* player, double ticks, bool rebuildBones) {
		if (!player || !UpdateClientsideAnimationOriginal || updatingClientAnimation)
			return false;
		if (player->GetClientNetworkable()->entIndex() == interfaces::engineClient->GetLocalPlayer()
			|| player->GetClientNetworkable()->IsDormant())
			return false;

		const double time = player->m_flSimulationTime();
		const double interval = interfaces::globalVars->interval_per_tick;
		if (!std::isfinite(time) || time <= 0.0 || !std::isfinite(interval) || interval <= 0.0
			|| !std::isfinite(ticks))
			return false;

		const double tick = std::floor(time / interval + 0.5);
		if (tick > (std::numeric_limits<int>::max)())
			return false;

		AnimationUpdateScope scope(*player, time, static_cast<int>(tick), std::clamp(std::floor(ticks + 0.5), 1.0, 24.0));
		UpdateClientsideAnimationOriginal(player);
		return !rebuildBones || rebuildAnimationBones(player);
	}

	void __fastcall UpdateClientsideAnimationHookFunc(CBasePlayer* self) {
		if (updatingClientAnimation)
			return;
		if (!globals::shouldFixAnimations || !self->IsPlayer()
			|| self->GetClientNetworkable()->entIndex() == interfaces::engineClient->GetLocalPlayer()
			|| self->GetClientNetworkable()->IsDormant())
			return UpdateClientsideAnimationOriginal(self);

		bool updateAllowed = true;
		double updateTicks = 1.0;
		...
		if (updateAllowed)
			updateClientSideAnimations(self, updateTicks);
	}
